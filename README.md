...
- 邮件配置：配置导出功能使用前端内置的 EmailJS 配置导出模板；模板中的「一键复制配置 JSON」按钮指向 GitHub Pages 上的复制中转页 `public/import.html`（邮件客户端禁用脚本，复制动作由该网页完成），配置内容经 base64url 编码放在链接 `#` 后的 fragment 中，不会上传服务器。
